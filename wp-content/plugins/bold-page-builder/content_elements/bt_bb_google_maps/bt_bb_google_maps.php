...
class bt_bb_google_maps extends BT_BB_Element {
	...
	function handle_shortcode( $atts, $content ) {
		...
		$output = '<div class="' . esc_attr( $this->shortcode ) . '_map ' . esc_attr( $class_master ) . '_map" id="' . esc_attr( $map_id ) . '"' . $style_height . '>';
		$output .= '</div>';
		$output .= $content;
		...
		$output_script .= ' document.addEventListener("readystatechange", function() { ';
			$output_script .= ' if ( !bt_bb_google_map_' . $map_id . '_init_finished && ( document.readyState === "interactive" || document.readyState === "complete" ) ) { ';
				...
				if ( $map_type == 'interactive' ) {
					$output_script .= ' bt_bb_gmap_init( "' . esc_attr( $map_id ) . '", ' . intval( $zoom ) . ', "' . $custom_style . '" );';	
				} else {
					$output_script .= ' bt_bb_gmap_init_static( "' . esc_attr( $map_id ) . '", ' . intval( $zoom ) . ', "' . $custom_style . '", ' . intval( $height ) . ', "' . esc_attr( $api_key ) . '" );';	
				}
	...
	function map_shortcode() {
		bt_bb_map( $this->shortcode, array( 'name' => esc_html__( 'Google Maps', 'bold-builder' ), 'description' => esc_html__( 'Google Map with custom content', 'bold-builder' ), 'container' => 'vertical', 'accept' => array( 'bt_bb_google_maps_location' => true ), 'icon' => $this->prefix_backend . 'icon' . '_' . $this->shortcode,
			'params' => array(
				...
				array( 'param_name' => 'custom_style', 'type' => 'textarea_object', 'heading' => esc_html__( 'Custom map style array (interactive map) or querystring (static image map)', 'bold-builder' ), 'description' => esc_html__( 'Find more custom styles at https://snazzymaps.com/ or https://mapstyle.withgoogle.com/. ', 'bold-builder' ) ),
